...
from .errors import FormatGridError, LogTypeError
...
class Grid:
    def __init__(self, width: int, height: int):
        ...
        self._data = numpy.array([[None] * self.width] * self.height)
    ...
    def __repr__(self):
        return "<Grid {}x{} created at {}>".format(
            self.width, self.height, repr(str(self.log.created_at))
        )
    ...
    def format_cells(self, text_align="left"):
        str_cells = [
            [str(repr(x)) if x is not None else NULL for x in y] for y in self._data
        ]
        ...
        if text_align == "left":
            for y in str_cells:
                yield gen(x + " " * (longest_cell_len - len(x)) for x in y)
        elif text_align == "right":
            for y in str_cells:
                yield gen(" " * (longest_cell_len - len(x)) + x for x in y)
        elif text_align == "center":
            for y in str_cells:
                yield gen(
                    " " * ((longest_cell_len - len(x)) / 2)
                    + x
                    + " " * ((longest_cell_len - len(x)) / 2)
                    for x in y
                )
        else:
            raise FormatGridError(
                f"{text_align} isn't an acceptable text alignement. "
                "Only left, right, or center are allowed."
            )
    ...
    def update_cell(self, x, y, new_value) -> None:
